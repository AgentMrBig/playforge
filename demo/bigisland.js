...
// Terrain collision cell size (m). Smaller = smoother collision surface. The
// heightfield collider (see attachTilePhysics) sampled from heightAt at this
// resolution replaced the old dense-trimesh collider (Ember's 2026-07-20 fine
// collider), which paid a ~21ms build per tile and hitched at every tile
// crossing. ?coarsecol=1 falls back to the coarse render-mesh collider for A/B.
const COLLISION_CELL = new URLSearchParams(location.search).get("coarsecol") ? 0 : 1.5;
function attachTilePhysics(tile, mesh) {
  if (tile.dead) return;
  const cols = [];
  // HEIGHTFIELD terrain collider (2026-07): a Rapier heightfield builds in ~0.5ms
  // vs the ~7ms BVH of the equivalent trimesh (+ ~14ms sampling), and every 128m
  // tile used to pay the full trimesh cost as it streamed in → a dropped frame at
  // each tile crossing = the jitter at speed. Same smooth interpolated surface,
  // sampled from heightAt at COLLISION_CELL resolution. ?coarsecol=1 A/B falls
  // back to the coarse render-mesh collider. Proven in the Map Lab (map.html).
  if (COLLISION_CELL > 0) {
    const n = Math.max(8, Math.round(tile.size / COLLISION_CELL));
    cols.push(phys.addHeightfield(tile.x0, tile.z0, tile.size, heightAt, { n, friction: 1.0 }));
  } else {
    cols.push(...phys.addMeshCollider(mesh));                                                               // ?coarsecol=1 A/B
  }
